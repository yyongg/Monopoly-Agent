...
class OpponentPool:
    ...
    Args:
        ...
        baseline_prob (float): Probability of returning the engine baseline
            (``None``) instead of a snapshot, keeping some baseline exposure.
    ...
    def __call__(self):
        """Samples an opponent policy for one episode (``None`` == baseline)."""
        if self._rng.random() < self.baseline_prob:
            return None
...
def make_selfplay_env(rank, seed, seat, reward_mode, max_turns, pool_dir,
                      baseline_prob, opp_deterministic):
